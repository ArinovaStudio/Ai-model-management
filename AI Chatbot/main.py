...
async def chat(request: ChatRequest):
    ...
    if "github" in q:
        ...
        if not user.githubProfile:
            return {"reply": f"{user.name} has no GitHub profile."}

        return {"reply": f"{user.name}'s GitHub: {user.githubProfile}"}
    ...
    if "payment" in q or "salary" in q or "payout" in q:
        ...
        if schedule:
            ...
            return {
                "reply":
                f"{user.name}'s payout:\nNext Date: {next_date}\nNext Amount: ₹{next_amount}\nTotal Paid: ₹{total_paid}"
            }

        if total_paid > 0:
            return {"reply": f"{user.name} total payout: ₹{total_paid}"}
