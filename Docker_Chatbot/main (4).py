...
async def handle_query(query: str, ctx: dict):
    ...
    if tbl == "workhours" and ("attendance" in raw_q or "report" in raw_q):
        ...
        #BUG 9 fixed
        absnt = [
            d for d in range(1, end_d + 1) 
            if d not in prs and date(yr, cur_m, d).weekday() < 5
        ]
    ...
    #   BUG 8 FIX — Meeting should return NEXT upcoming meeting only
    if tbl == "meeting":
        ...
        try:
            ...
            if meetings:
                m = meetings[0]
                return f"Next meeting: {getattr(m, 'title', 'Meeting')} at {m.scheduledAt}"
